...
# Título Principal
st.markdown(
    "<h1 class='main-header'>📊 Painel Analítico de Vendas & Logística - Northwind Traders</h1>",
    unsafe_allow_html=True,
)
...
def check_password():
    ...
    if not dashboard_user or not dashboard_password:
        st.warning(
            "⚠️ Credenciais de acesso não configuradas no arquivo de ambiente (.env)."
        )
        st.stop()
...
if not os.path.exists(db_path):
    st.warning(
        "⚠️ Banco de dados analítico local `northwind.duckdb` não localizado. Por favor, execute o pipeline de dados via terminal primeiro."
    )
    st.stop()
...
with tab_finance:
    ...
    if df_fin is not None and not df_fin.empty:
        ...
        with col_g1:
            st.markdown(
                "##### Comparativo de Receita Bruta vs Lucro Líquido Real por Categoria"
            )
            fig_fin_bar = go.Figure(
                data=[
                    go.Bar(
                        name="Receita Bruta",
                        x=df_fin["category_name"],
                        y=df_fin["gross_revenue"],
                        marker_color="#3B82F6",
                    ),
                    go.Bar(
            ...
            fig_fin_bar.update_layout(
                barmode="group", height=400, margin=dict(t=20, b=20, l=20, r=20)
            )
...
with tab_logistic:
    ...
    if df_log is not None and not df_log.empty:
        ...
        with col_g1:
            st.markdown(
                "##### Desempenho Logístico e Taxa de Atraso por País de Destino"
            )
            # Agrupa dados por país
            df_country = (
                df_log.groupby("destination_country")
                .agg(
                    {
                        "total_orders": "sum",
            ...
            df_country["late_rate_pct"] = (
                df_country["late_orders_count"] / df_country["total_orders"]
            ) * 100
            ...
            fig_country = px.bar(
                ...
                labels={
                    "late_rate_pct": "Taxa de Atraso (%)",
                    "destination_country": "País de Destino",
                    "avg_lead_time_days": "Lead Time Médio",
                },
        ...
        with col_g2:
            ...
            fig_deviation = px.bar(
                ...
                labels={
                    "avg_deviation_days": "Desvio de Entrega (Dias)",
                    "destination_region": "Região de Destino",
                },
                height=350,
            )
...
with tab_shippers:
    ...
    if df_ship is not None and not df_ship.empty:
        ...
        with col_s1:
            st.markdown(
                "##### Custo de Frete Médio vs Lead Time Médio por Transportadora"
            )
            df_ship_agg = (
                df_ship.groupby("shipper_name")
                .agg(
                    {
                        "total_orders": "sum",
            ...
            df_ship_agg["avg_freight_cost"] = (
                df_ship_agg["total_freight_cost"] / df_ship_agg["total_orders"]
            )
            df_ship_agg["late_rate_pct"] = (
                df_ship_agg["late_orders_count"] / df_ship_agg["total_orders"]
            ) * 100
            ...
            fig_bubble = px.scatter(
                ...
                labels={
                    "avg_lead_time_days": "Prazo de Entrega Médio (Dias)",
                    "avg_freight_cost": "Custo Médio de Frete (R$)",
                },
                size_max=60,
                height=400,
            )
        ...
        with col_s2:
            ...
            fig_ship_late = px.bar(
                ...
                labels={
                    "late_rate_pct": "Taxa de Atraso (%)",
                    "shipper_name": "Transportadora",
                },
                color_discrete_sequence=px.colors.qualitative.Set2,
                height=400,
            )
            fig_ship_late.update_layout(
...
with tab_employees:
    ...
    if df_emp is not None and not df_emp.empty:
        ...
        with col_e1:
            st.markdown(
                "##### Correlação: Volume de Pedidos vs Tempo Médio de Processamento"
            )
            fig_emp_scatter = px.scatter(
                ...
                labels={
                    "avg_processing_time_days": "Tempo Médio de Envio (Dias)",
                    "total_orders_processed": "Pedidos Processados (Volume)",
                    "job_title": "Cargo",
                },
                size_max=40,
                height=400,
        ...
        with col_e2:
            ...
            fig_emp_profit = px.bar(
                ...
                orientation="h",
                labels={
                    "total_profit_generated": "Lucro Gerado (R$)",
