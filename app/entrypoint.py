...
def main():
    ...
    cmd = ["streamlit", "run", "app/main.py", "--server.port", "8501", "--server.address", "0.0.0.0"]  # nosec B104
    ...
    if enable_ssl:
        ...
        if not os.path.exists(cert_file) or not os.path.exists(key_file):
            print("[BOOT] Certificados não localizados. Gerando chaves autoassinadas via OpenSSL...")
            
            # Verifica se openssl está instalado
            if not shutil.which("openssl"):
                print("[ERRO] Binário 'openssl' não encontrado. Instale o openssl ou desative o SSL.")
                sys.exit(1)
